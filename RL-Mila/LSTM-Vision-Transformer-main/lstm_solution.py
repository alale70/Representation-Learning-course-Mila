...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        output, state = self.lstm(embed, hidden_states)
        out = self.classifier(output)
        log = F.log_softmax(out, dim=2)
        # ==========================
        return log, state
    ...
    def loss(self, log_probas, targets, mask):
        ...
        # ==========================
        # TODO: Write your code here
        output1=torch.gather(log_probas,2,targets[...,None])
        output11=torch.squeeze(output1,dim=2)
        output2=torch.mul(output11,mask)
        output3=torch.sum(output2,1)
        mask2=torch.sum(mask,1)
        output44=output3/mask2
        output5=-1*output44
        output5=torch.mean(output5)
        # ==========================
        return output5
