...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.query_matrix = nn.Linear(self.num_heads*self.head_size , self.num_heads*self.head_size ,bias=True) 
        self.key_matrix = nn.Linear(self.num_heads*self.head_size  , self.num_heads*self.head_size, bias=True)
        self.value_matrix = nn.Linear(self.num_heads*self.head_size ,self.num_heads*self.head_size , bias=True)
        self.out = nn.Linear(self.num_heads*self.head_size ,self.num_heads*self.head_size)
    ...
    def get_attention_weights(self, queries, keys):
        ...
        QK_T = torch.matmul(queries, k_transpose)  
      
        #divising by square root of key dimension
        QK_T = QK_T / math.sqrt(self.head_size) 

        #applying softmax
        scores = F.softmax(QK_T, dim=-1)
    ...
    def apply_attention(self, queries, keys, values):
        ...
        scores = self.get_attention_weights(queries, keys)
        
        #mutiply with value matrix
        scores = torch.matmul(scores, values) 
        
        #concatenated output
        concat_head = self.merge_heads(scores)
    ...
    def split_heads(self, tensor):
        ...
        dim = int(tensor.size(2)/self.num_heads)
        
        split_heads = tensor.view(batch_size, self.sequence_length, self.num_heads, dim)
        split_heads = split_heads.transpose(1,2)
        # ==========================
        return split_heads
    ...
    def merge_heads(self, tensor):
        ...
        merge_heads = tensor.transpose(1,2).contiguous().view(batch_size, self.sequence_length, self.num_heads*dim)
        # ==========================
        return merge_heads
    ...
    def forward(self, hidden_states):
        ...
        Key = self.split_heads(self.key_matrix(hidden_states))
        Query = self.split_heads(self.query_matrix(hidden_states))
        Value = self.split_heads(self.value_matrix(hidden_states))
        
        Atten_out = self.apply_attention(Query, Key, Value)
    
        output = self.out(Atten_out) 
        # ==========================
        return output
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.attn(attention_outputs)
