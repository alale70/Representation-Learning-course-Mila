...
class SimSiam(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        Note that the outputs are different if stop_gradient is True or False
        """
    ...
    def loss (self, p1,p2,z1,z2, similarity_function='CosineSimilarity'):
        ...
        """
        D = CosineSimilarity()
        return - 0.5*D.forward(p1, z2).mean() - 0.5*D.forward(p2, z1).mean()
...
class CosineSimilarity(Module):
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        COMPLETE ME. DONT MODIFY THE PARAMETERS OF THE FUNCTION. Otherwise, tests might fail.
        """
        dot_product = (x1*x2).sum(dim=self.dim)
        norm1 = torch.sqrt((x1*x1).sum(dim=self.dim))
        norm2 = torch.sqrt((x2*x2).sum(dim=self.dim))
        norm12 = norm1*norm2
        epss = self.eps*torch.ones_like(norm12)
        norm12 = torch.maximum(norm12, epss)

        cos_sim = dot_product/norm12
